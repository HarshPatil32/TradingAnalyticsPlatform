...
_ENV_VAR_MANIFEST = (
    {
        "name": "PORT",
        "required": False,
        "description": "Port Flask listens on (defaults to 5001)",
    },
    {
        "name": "FLASK_DEBUG",
        "required": False,
...
def validate_env_vars():
    ...
    for var in _ENV_VAR_MANIFEST:
        if not os.environ.get(var["name"]):
            if var["required"]:
                logging.warning(
                    "Missing required env var: %s (%s)", var["name"], var["description"]
                )
                missing_required.append(var["name"])
            else:
                logging.info(
                    "Optional env var not set: %s (%s)", var["name"], var["description"]
                )
...
def _parse_allowed_origins() -> "str | list[str]":
    ...
    for origin in origins:
        if not origin.startswith(("http://", "https://")):
            _logger.warning(
                "ALLOWED_ORIGINS entry looks invalid (expected http/https): %s", origin
            )
...
def _build_cors_origins(parsed: "str | list[str]"):
    ...
    for origin in parsed:
        ...
        if match:
            ...
            cors_origins.append(
                re.compile(
                    rf"^{scheme}{re.escape(slug)}(-[a-zA-Z0-9-]+)*\.vercel\.app$"
                )
            )
...
def request_too_large(error):
    ...
    return (
        jsonify(
            {"error": f"Payload too large. Maximum allowed size is {limit_mb} MB."}
        ),
        413,
    )
...
def MACD_strategy():
    if not TRADING_MODULES_AVAILABLE:
        return (
            jsonify(
                {"error": "Trading modules not available. Please check server setup."}
            ),
            500,
        )
    ...
    try:
        ...
        if not stocks or not start_date_str or not end_date_str:
            return (
                jsonify(
                    {
                        "error": "Missing required parameters: stocks, start_date, end_date"
                    }
                ),
                400,
            )
        ...
        try:
            ...
            return (
                jsonify({"error": f"Invalid date format. Use YYYY-MM-DD: {str(e)}"}),
                400,
            )

        logger.info(
            f"Processing MACD strategy for stocks: {stock_list}, dates: {start_date_str} to {end_date_str}, optimize: {optimize}"
        )
        ...
        if optimize:
            ...
            str_result, _ = backtest_strategy_MACD(
                stock_list, start_date_dt, end_date_dt, initial_balance
            )
            formatted_result = str_result.replace("\n", "<br />")
...
def spy_investment():
    ...
    try:
        ...
        if not start_date_str or not end_date_str:
            return (
                jsonify({"error": "Missing required parameters: start_date, end_date"}),
                400,
            )
        ...
        try:
            ...
            return (
                jsonify(
                    {
                        "error": "Invalid date format. Please provide dates in ISO format (YYYY-MM-DD)."
                    }
                ),
                400,
            )
        ...
        logger.info(
            f"Processing SPY investment for dates: {start_date_str} to {end_date_str}"
        )
        ...
        if (
            isinstance(final_balance, tuple)
            and len(final_balance) == 2
            and isinstance(final_balance[1], int)
        ):
            error_msg, status = final_balance
            return jsonify({"error": error_msg}), status

        monthly_data = generate_spy_monthly_performance(
            start_date, end_date, initial_balance
        )

        return (
            jsonify(
                {"final_balance": final_balance, "monthly_performance": monthly_data}
            ),
            200,
        )
...
sys.modules[__name__].get_spy_investment = get_spy_investment  # type: ignore[attr-defined]
sys.modules[__name__].generate_spy_monthly_performance = (  # type: ignore[attr-defined]
    generate_spy_monthly_performance
)
    ...
    try:
        ...
        if timeframe not in valid_timeframes:
            return (
                jsonify(
                    {
                        "error": "The timeframe isn't valid, please choose short, medium or long"
                    }
                ),
                400,
            )
        ...
        if max_stocks < 1 or max_stocks > 10:
            return jsonify(
                {"error": "Please choose a number between 1 and 10 inclusive"}
            )
        ...
        if not selected_stocks:
            return (
                jsonify(
                    {
                        "error": "No suitable stocks found with your criteria. Please try different timeframe or risk level settings."
                    }
                ),
                400,
            )
...
def auto_trade():
    ...
    try:
        ...
        if timeframe not in valid_timeframes:
            return (
                jsonify({"error": "Invalid timeframe. Choose: short, medium, long"}),
                400,
            )
        ...
        logger.info(
            f"Auto-trading: timeframe={timeframe}, risk={risk}, dates={start_date_str} to {end_date_str}"
        )
        ...
        response_data = {
            "auto_selection": {
                ...
                "timeframe": timeframe,
                "risk": risk,
                "total_candidates_screened": len(selected_stocks)
                * 10,  # Rough estimate
            },
            "trading_results": {
    ...
    try:
        if "file" in request.files:
            ...
            try:
                ...
                return (
                    jsonify(
                        {
                            "error": "The filename is invalid. Please rename your file and try again."
                        }
                    ),
                    400,
                )
            ...
            return (
                jsonify(
                    {
                        "error": "No file received. Please upload a CSV file or send csv_data in the request body."
                    }
                ),
                400,
            )
        ...
        if commission_per_trade is not None:
            ...
            if commission_per_trade < 0:
                return (
                    jsonify(
                        {"error": "commission_per_trade must be a non-negative number."}
                    ),
                    400,
                )
            result = analyze_uploaded_trades(
                csv_data, commission_per_trade=commission_per_trade
            )
        else:
