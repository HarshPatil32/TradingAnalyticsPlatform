"""Tests for parse_options_summary() in options_analyzer."""

import pytest

from options_analyzer import parse_options_summary
...
class TestParseOptionsSummaryHappyPath:
    def test_returns_dict(self):
        assert isinstance(parse_options_summary(VALID_CSV), dict)

    def test_initial_capital_is_float(self):
        result = parse_options_summary(VALID_CSV)
    ...
    def test_start_date_preserved_as_string(self):
        assert parse_options_summary(VALID_CSV)["start_date"] == "2024-01-01"

    def test_end_date_preserved_as_string(self):
        assert parse_options_summary(VALID_CSV)["end_date"] == "2024-12-31"
...
class TestParseOptionsSummaryColumnHandling:
    def test_mixed_case_headers(self):
        csv_data = "Initial_Capital,Final_Balance,Num_Trades,Win_Rate,Start_Date,End_Date\n10000,12000,42,0.6,2024-01-01,2024-12-31\n"
        assert parse_options_summary(csv_data)["num_trades"] == 42
...
class TestParseOptionsSummaryMissingFields:
    ...
    def test_missing_required_field_raises(self, field):
        with pytest.raises(ValueError, match="missing required fields"):
            parse_options_summary(self._drop_col(field))
...
class TestParseOptionsSummaryWinRate:
    ...
    def test_above_one_raises(self):
        with pytest.raises(ValueError, match="win_rate"):
            parse_options_summary(self._make_csv("1.01"))

    def test_negative_raises(self):
        with pytest.raises(ValueError, match="win_rate"):
            parse_options_summary(self._make_csv("-0.01"))

    def test_non_numeric_raises(self):
        with pytest.raises(ValueError, match="win_rate"):
            parse_options_summary(self._make_csv("abc"))

    def test_percentage_format_raises(self):
        with pytest.raises(ValueError, match="win_rate"):
            parse_options_summary(self._make_csv("60"))


class TestParseOptionsSummaryDates:
    def _make_csv(self, start: str, end: str) -> str:
        return f"initial_capital,final_balance,num_trades,win_rate,start_date,end_date\n10000,12000,42,0.6,{start},{end}\n"

    def test_slash_format_start_raises(self):
        with pytest.raises(ValueError, match="start_date"):
            parse_options_summary(self._make_csv("01/01/2024", "2024-12-31"))

    def test_slash_format_end_raises(self):
        with pytest.raises(ValueError, match="end_date"):
            parse_options_summary(self._make_csv("2024-01-01", "12/31/2024"))

    def test_missing_leading_zero_start_raises(self):
        with pytest.raises(ValueError, match="start_date"):
            parse_options_summary(self._make_csv("2024-1-1", "2024-12-31"))

    def test_missing_leading_zero_end_raises(self):
        with pytest.raises(ValueError, match="end_date"):
            parse_options_summary(self._make_csv("2024-01-01", "2024-1-31"))

    def test_start_after_end_raises(self):
        with pytest.raises(ValueError, match="start_date"):
            parse_options_summary(self._make_csv("2024-12-31", "2024-01-01"))

    def test_invalid_month_raises(self):
        with pytest.raises(ValueError, match="start_date"):
            parse_options_summary(self._make_csv("2024-13-01", "2024-12-31"))

    def test_error_message_includes_bad_value(self):
        bad = "01/01/2024"
        with pytest.raises(ValueError, match=bad):
            parse_options_summary(self._make_csv(bad, "2024-12-31"))


class TestParseOptionsSummaryNumericFields:
    def _make_csv(
    ...
    def test_zero_initial_capital_raises(self):
        with pytest.raises(ValueError, match="initial_capital"):
            parse_options_summary(self._make_csv(initial="0"))

    def test_negative_initial_capital_raises(self):
        with pytest.raises(ValueError, match="initial_capital"):
            parse_options_summary(self._make_csv(initial="-1000"))

    def test_non_numeric_initial_capital_raises(self):
        with pytest.raises(ValueError, match="initial_capital"):
            parse_options_summary(self._make_csv(initial="abc"))

    def test_zero_final_balance_raises(self):
        with pytest.raises(ValueError, match="final_balance"):
            parse_options_summary(self._make_csv(final="0"))

    def test_zero_num_trades_raises(self):
        with pytest.raises(ValueError, match="num_trades"):
            parse_options_summary(self._make_csv(trades="0"))

    def test_fractional_num_trades_raises(self):
        with pytest.raises(ValueError, match="num_trades"):
            parse_options_summary(self._make_csv(trades="42.5"))

    def test_non_numeric_num_trades_raises(self):
        with pytest.raises(ValueError, match="num_trades"):
            parse_options_summary(self._make_csv(trades="abc"))


class TestParseOptionsSummaryMultipleRows:
    def test_two_data_rows_raises(self):
        ...
        with pytest.raises(ValueError, match="exactly one data row"):
            parse_options_summary(csv_data)


class TestParseOptionsSummaryWhitespaceCells:
