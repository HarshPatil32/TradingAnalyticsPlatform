...
DEFAULT_COMMISSION_PER_TRADE: float = (
    0.00  # Commission in USD per trade leg — $0 for commission-free brokers (Robinhood, Webull, etc.)
)
...
MIN_CLOSED_TRADES_FOR_CONCLUSIONS: int = (
    30  # Minimum closed trades for statistically reliable results
)
...
def _normalise_summary(summary: dict) -> list[NormalisedTrade]:
    ...
    final_balance = float(
        summary.get("final_balance", initial_capital) or initial_capital
    )
...
def calculate_slippage(
    ...
    if (
        not isinstance(slippage_pct, (int, float))
        or slippage_pct < 0
        or slippage_pct > 1
    ):
...
def calculate_bid_ask_spread(
    ...
    total = sum(
        cast(float, entry["round_trip_spread_usd"]) for entry in per_trade_breakdown
    )
    num = len(normalised)
...
def calculate_taxes(
    ...
    effective_rate = (
        (total_tax / (short_taxable + long_taxable))
        if (short_taxable + long_taxable) > 0
        else 0.0
    )
...
def calculate_win_rate(trades: Any) -> dict:
    ...
    num_wins = sum(1 for nt in closed if (nt.profit or 0) > 0)
...
def calculate_real_costs(
    ...
    if _is_summary(trades):
        ...
        normalised, _ = _to_normalised(trades)
        gross_profit_usd = sum(nt.profit for nt in normalised if nt.profit is not None)
        gross_return_pct = (
            (gross_profit_usd / account_size * 100) if account_size else 0.0
        )
    ...
    total_cost_pct = (
        (total_trading_costs_usd / account_size * 100) if account_size else 0.0
    )
    total_all_costs_pct = (
        (total_all_costs_usd / account_size * 100) if account_size else 0.0
    )
    ...
    if gross_return_pct == 0:
        warnings.append(
            "Gross return is 0% — verify that trades or summary data are correctly formatted."
        )
    if total_trading_costs_usd > abs(gross_profit_usd) * 0.5:
        warnings.append(
            "Trading costs exceed 50% of gross profit — trade frequency may be too high."
        )
    if after_costs_and_tax_pct < 0 < gross_return_pct:
        warnings.append(
            "Strategy is profitable gross but unprofitable after all costs and taxes."
        )
    ...
    return {
        ...
        "cost_summary": {
            ...
            "breakdown_pct": {
                "commissions": (
                    round(comm["total_commission_usd"] / account_size * 100, 4)
                    if account_size
                    else 0
                ),
                "slippage": (
                    round(slip["total_slippage_usd"] / account_size * 100, 4)
                    if account_size
                    else 0
                ),
                "bid_ask_spread": (
                    round(spread["total_spread_usd"] / account_size * 100, 4)
                    if account_size
                    else 0
                ),
                "taxes": (
                    round(total_tax_usd / account_size * 100, 4) if account_size else 0
                ),
            },
        },
        "adjusted_returns": {
